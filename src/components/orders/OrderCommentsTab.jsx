  ...
  return (
    <Box sx={{ mb: 3 }}>
      ...
      <Box sx={{ display: 'flex', mt: 1 }}>
        ...
        >
          {(comment.user || comment.author || 'User').charAt(0)}
        </Avatar>
        <Box sx={{ flexGrow: 1 }}>
          <Typography variant="subtitle2">{comment.user || comment.author || 'Unknown User'}</Typography>
          <Typography variant="body2">{comment.comment || comment.text || comment.content || ''}</Typography>
        </Box>
      </Box>
    </Box>
  );
