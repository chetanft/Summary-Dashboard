...
export const transformDashboardData = (data) => {
  ...
  const secondaryKPIs = data.kpis.filter(kpi => kpi.id !== 'freight_budget_actual')
    .map(kpi => ({
      ...
      title: kpi.label,
      value: kpi.value,
      formattedValue: formatKPIValue(kpi.value, kpi.unit),
      target: kpi.target,
      formattedTarget: kpi.target ? formatKPIValue(kpi.target, kpi.unit) : 'N/A',
      color: kpi.color,
      unit: kpi.unit,
      trend: kpi.trend || []
    }));
  ...
  const transformedHeroKPI = heroKPI ? {
    title: heroKPI.label,
    actual: heroKPI.value,
    formattedActual: formatKPIValue(heroKPI.value, heroKPI.unit),
    projected: heroKPI.projected,
    formattedProjected: heroKPI.projected ? formatKPIValue(heroKPI.projected, heroKPI.unit) : 'N/A',
    budget: heroKPI.budget,
    formattedBudget: formatKPIValue(heroKPI.budget, heroKPI.unit),
    color: heroKPI.color,
    unit: heroKPI.unit,
